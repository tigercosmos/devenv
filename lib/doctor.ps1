# lib/doctor.ps1 — the `devenv doctor` check on Windows. Dot-sourced by
# scripts\devenv.ps1 after lib\common.ps1. Sets $rc; the caller exits with it.
$rc = 0
...
foreach ($dir in @((Join-Path $HOME '.codex\skills'), (Join-Path $HOME '.agents\skills'), (Join-Path $HOME '.cursor\skills'))) {
    ...
    if ($missing) { Warn "$dir is missing: $($missing -join ', ')  (run: devenv sync-skills)" }
    if ($stale) { Warn "$dir has a local copy, not a link, of: $($stale -join ', ')  (devenv sync-skills -Force)" }
}
